...
void DisplayParScr()
{
	...
	Lcd_Clr(6);	
	Lcd_Fill_Box(0, 0, 320*2,34*2, 13);	
	Lcd_DispHZ_On_Button(100*2,5*2,220*2,20*2,4,2,13,3,0,1,0,0,Info);    //显示参数设置
	Lcd_Fill_Box(0, 206*2, 320*2,240*2, 13);

	Left = 10*2;
	Top = 100;					//43*2;

	Lcd_Display_String(Left,Top,6,2,6,2,TongXunDuanKou,0);  //通讯端口设定:
	Lcd_Fill_Box(162*2,94,214*2,(94+36),13);

	j=new_NO[0];
	length=strlen(Str[0][j]);
   	Lcd_DispAsc_On_Button(162*2,94,214*2,130,length,13,2,1,0,2,Str[0][j]);
	num[0]=3;       
	num[1]=26;                                                   //:
	Lcd_Display_String(168,104,1,2,6,0,num,0);

	Top = 100+48;					// 2*(34*2+9);

	Lcd_Display_String(Left,Top,5,2,6,2,BoTeLvSheDing,0);                  //波特率设定
	j=new_NO[1];
	length=strlen(Str[1][j]);
	Lcd_DispAsc_On_Button(162*2,142,214*2,142+36,length,6,2,1,0,2,Str[1][j]);
	Lcd_Display_String(144,152,1,2,6,0,num,0);

	Top = 100+48*2;					// 2*(34*3+9);

	Lcd_Display_String(Left,Top,7,2,6,2,JiOuJiaoYan,0);                //“奇偶校验:
	j=new_NO[2];
	length=strlen(Str[2][j]);
	Lcd_DispAsc_On_Button(162*2, 190, 2*214, (190+36),length,6,2,1,0,2,Str[2][j]);
	Lcd_Display_String(192,200,1,2,6,0,num,0);

	Top = 100+48*3;					// 2*(34*4+9);

	Lcd_Display_String(Left,Top,5,2,6,2,ShuJuWei,0);                    // 数据位:
	j=new_NO[3];
	length=strlen(Str[3][j]);
	Lcd_DispAsc_On_Button(162*2, 242, 2*214, (242+36),length,6,2,1,0,2,Str[3][j]);
	Lcd_Display_String(144,248,1,2,6,0,num,0);

	Top = 100+48*4;					// 2*(34*5+9);

	Lcd_Display_String(Left,Top,5,2,6,2,TingZhiWei,0);                 // 停止:
	j=new_NO[4];
	length=strlen(Str[4][j]);
	Lcd_DispAsc_On_Button(162*2, 286, 2*214, (286+36),length,6,2,1,0,2,Str[4][j]);
	Lcd_Display_String(144,292,1,2,6,0,num,0);

	Top = 100+48*5;					// 2*(34*5+9);

	Lcd_Display_String(Left,Top,4,2,6,2,SrcSave,0);                 // 屏保:
	j=new_NO[5];
	length=strlen(Str[5][j]);
	Lcd_DispAsc_On_Button(162*2, 334, 2*214, (334+36),length,6,2,1,0,2,Str[5][j]);
	Lcd_Display_String(120,340,1,2,6,0,num,0);


	Lcd_Draw_VLine (35*2, 2*205, 2*216, 13, 3);
	
	Left=220*2;
	Top = 2*(34+13.5);

	Lcd_Display_String(Left,2*43,2,2,6,2,XuanZe,0);                  // 选择
	Lcd_Button(224*2, 2*61,2*264, 2*86,10, 2,RAISE);
	num[0]=1;
	num[1]=92;
	Lcd_DispHZ_On_Button(224*2, 2*61,2*264, 2*86,1,2,10,1,0,0,0,0,num);    //显示 up

	Lcd_Button(2*273, 2*61,2*313, 2*86,10,2,RAISE);
	num[0]=1;
	num[1]=93;
	Lcd_DispHZ_On_Button(2*273, 2*61,2*313, 2*86,1,2,10,1,0,0,0,0,num);    //显示 donw

        Top = 2*(34+43+13.5);

	Lcd_Display_String(Left,Top,2,2,6,2,XiuGai,0);                  // 修改
	Lcd_Button(2*224, Top+18*2, 2*264, Top+43*2,10, 2,RAISE);
	num[0]=1;
	num[1]= 91 ;
	Lcd_DispHZ_On_Button(2*224, Top+18*2, 2*264, Top+43*2,1,2,10,1,0,0,0,0,num);    //显示 +

	Lcd_Button(2*273, Top+18*2, 2*313, Top+43*2,10, 2,RAISE);
	num[0]=1;
	num[1]=90 ;
	Lcd_DispHZ_On_Button(2*273, Top+18*2, 2*313, Top+43*2,1,2,10,1,0,0,0,0,num);    //显示 -

	Top = 2*(34+43*2+13.5);

   	Lcd_Button(Left+8, Top+10, 2*313, Top+70,10, 2,RAISE);
	Lcd_DispHZ_On_Button(Left+8, Top+10, 2*313, Top+70,2,2,10,2,0,0,0,3,QueDing);    //显示 确定

	Top = 2*(34+43*3+13.5);

   	Lcd_Button(Left+8, Top-10, 2*313, Top+50,10,2,RAISE);
	Lcd_DispHZ_On_Button(Left+8, Top-10, 2*313, Top+50,2,2,10,2,0,0,0,3,QuXiao);    //显示 取消
	...
	while(1)
	{
		...
		{		
			if(X_result>=224*2&&X_result<=313*2&&Y_result>=138.5*2&&Y_result<=2*168.5)   //button 确定
			{
				...
				Left = 220*2;
				Top = 2*(34+43*2+13.5);
				Lcd_Button(Left+8, Top+10, 2*313, Top+70,10, 2,INSERT);
	            		Lcd_DispHZ_On_Button(Left+8, Top+10, 2*313, Top+70,2,2,10,2,0,0,0,3,QueDing); 
			...
			if(X_result>=273*2&&X_result<=2*313&&Y_result>=2*61&&Y_result<=2*86)   //button down  you
			{
				...
				Num[0]=1;
				Num[1]=93;
				Lcd_Button(2*273, Top,2*313, 2*86,10,2,INSERT);
				Lcd_DispHZ_On_Button(2*273, Top,2*313, 2*86,1,2,10,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
				...
				Lcd_Button(2*273, Top,2*313, 2*86,10,2,RAISE);
				Lcd_DispHZ_On_Button(2*273, Top,2*313, 2*86,1,2,10,1,0,0,0,0,Num); 
			...
			if(X_result>=2*224&&X_result<=2*264&&Y_result>=2*61&&Y_result<=2*86)   //button up  zuo
			{
				...
				Num[0]=1;
				Num[1]=92;
				Lcd_Button(2*224, 2*61,2*264, 2*86,10, 2,INSERT);
				Lcd_DispHZ_On_Button(2*224, 2*61,2*264, 2*86,1,2,10,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
				...
				Lcd_Button(2*224, 2*61,2*264, 2*86,10, 2,RAISE);
				Lcd_DispHZ_On_Button(2*224, 2*61,2*264, 2*86,1,2,10,1,0,0,0,0,Num); 			
			...
			if(X_result>=2*224&&X_result<=2*264&&Y_result>=2*108.5&&Y_result<=2*133.5)   //button -
			{
				...
				Num[0]=1;
				Num[1]=91;
			   	 Lcd_Button(2*224, Top+18*2, 2*264, Top+43*2,10, 2,INSERT);
				Lcd_DispHZ_On_Button(2*224, Top+18*2, 2*264, Top+43*2,1,2,10,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
			...
			if(X_result>=2*273&&X_result<=2*313&&Y_result>=2*108.5&&Y_result<=2*133.5)   //button +
			{
				...
				Num[0]=1;
				Num[1]=90;
				Lcd_Button(2*273, Top+36, 2*313, Top+43*2,10, 2,INSERT);
			 	Lcd_DispHZ_On_Button(2*273, Top+36, 2*313, Top+43*2,1,2,10,1,0,0,0,0,Num); 
				Beep(1);
				for(w=0;w<BEEP_DELAY;w++);
				Beep(0);
				...
				Lcd_Button(2*273, Top+36, 2*313, Top+43*2,10, 2,RAISE);
				Lcd_DispHZ_On_Button(2*273, Top+36, 2*313, Top+43*2,1,2,10,1,0,0,0,0,Num); 
			...
			if(X_result>=2*224&&X_result<=2*313&&Y_result>=2*171.5&&Y_result<=2*201.5)   //button 取消
			{
				...
				Left = 2*220;
				Top = 2*(34+43*3+13.5);
				Lcd_Button(Left+8, Top-10, 2*313, Top+50,10,2,INSERT);
				Lcd_DispHZ_On_Button(Left+8, Top-10, 2*313, Top+50,2,2,10,2,0,0,0,3,QuXiao);    //显示 取消
...
void DrawBackColor(char flag,char old_i,char new_i)
{
	...
	if(flag==3)	// button  Up Down
	{
		if(new_i!=old_i)
		{
			Lcd_Fill_Box( 2*162,top1 ,2*214,bottom1,6);   
  			j=new_NO[old_i];
			length=strlen(Str[old_i][j]);
			Lcd_DispAsc_On_Button(2*162,top1 ,2*214,bottom1,length,6,2,1,0,2,Str[old_i][j]);
		}
	}
	
	Lcd_Fill_Box( 2*162,top ,2*214,bottom,13);                              //画背景
	
	j=new_NO[new_i];
	length=strlen(Str[new_i][j]);
	Lcd_DispAsc_On_Button(2*162,top ,2*214,bottom,length,13,2,1,0,2,Str[new_i][j]);
}
...
void DisplayScrCheck(char CurrentSrcNum)
{
	...
	Lcd_Clr(6);	
	Lcd_Fill_Box(0, 0, 320*2,34*2, 13);	
	Lcd_DispHZ_On_Button(2*100,2*5,2*220,2*20,4,2,13,3,0,1,0,0,Info);    //显示画面检查
	Lcd_Fill_Box(0, 206*2, 2*320,2*240, 13);
	Lcd_DispHZ_On_Button(0, 206*2, 2*320,2*234,15,2,13,1,0,0,0,0,Tishi);  

	Lcd_DispHZ_On_Button(0,2*40,2*320,2*56,9,2,6,2,0,1,0,0,All);  //应用程序共有画面数:
	num[0]=3;
	num[1]=26;
	Lcd_Display_String(430,80,1,2,6,0,num,0);                     	//:
	sprintf(str,"%d",g_TotalSrceenNum);
	Lcd_DispAsc_On_Button(440,79,440+50,2*56,strlen(str),6,2,1,0,0,str);

	Lcd_DispHZ_On_Button(0,60*2,2*320,2*76,7,2,6,2,0,1,0,0,Current); //当前检查画面号:
	Lcd_Display_String(406,120,1,2,6,0,num,0);                     	//:
	sprintf(str,"%d",CurrentSrcNum);
	Lcd_Fill_Box(120*2, 2*86, 2*200, 2*117,13);       
	Lcd_DispAsc_On_Button(120*2, 2*86, 2*200, 2*117,strlen(str),13,2,1,0,0,str);

	Lcd_Button(50*2, 2*87, 2*100, 2*117,10, 2,RAISE);
	num[0]=1;
	num[1]= 92;
	Lcd_DispHZ_On_Button(50*2, 2*87, 2*100, 2*117,1,2,10,1,0,0,0,0,num);    //显示 +

	
	Lcd_Button(220*2, 2*87, 2*270, 2*117,10, 2,RAISE);
	num[0]=1;
	num[1]= 93;
	Lcd_DispHZ_On_Button(220*2, 2*87, 2*270, 2*117,1,2,10,1,0,0,0,0,num);    //显示 -

	Lcd_Button(120*2, 2*124, 2*200, 2*158,10, 2,RAISE);
	Lcd_DispHZ_On_Button(120*2, 2*124, 2*200, 2*158,2,2,10,2,0,0,0,0,JianCha);    //检查

	Lcd_Button(120*2, 2*165, 2*200, 2*199,10, 2,RAISE);
	Lcd_DispHZ_On_Button(120*2, 2*165, 2*200, 2*199,2,2,10,2,0,0,0,0,FanHui);    //返回
...
void ScrCheckScan(void)
{
        ...
        while(1)
	{
		...
		if(flag==1)
		{
			...
			{
				//rINTMSK|=BIT_EINT1;
				Lcd_Button(2*50, 2*87, 2*100, 2*117,10, 2,INSERT);
				num[0]=1;
				num[1]= 92;
				Lcd_DispHZ_On_Button(2*50, 2*87, 2*100, 2*117,1,2,10,1,0,0,0,0,num);    //显示 +
				...
				Lcd_Button(2*50, 2*87, 2*100, 2*117,10, 2,RAISE);
				Lcd_DispHZ_On_Button(2*50, 2*87, 2*100, 2*117,1,2,10,1,0,0,0,0,num);    //显示 +
			...
			{
				//rINTMSK|=BIT_EINT1;
				Lcd_Button(2*220, 2*87, 2*270, 2*117,10, 2,INSERT);
				num[0]=1;
				num[1]= 93;
				Lcd_DispHZ_On_Button(2*220, 2*87, 2*270, 2*117,1,2,10,1,0,0,0,0,num);    //显示 -
				...
				Lcd_Button(2*220, 2*87, 2*270, 2*117,10, 2,RAISE);
				Lcd_DispHZ_On_Button(2*220, 2*87, 2*270, 2*117,1,2,10,1,0,0,0,0,num);    //显示 -
			...
			{
				//rINTMSK|=BIT_EINT1;
				Lcd_Button(2*120, 2*124, 2*200, 2*158,10, 2,INSERT);
				Lcd_DispHZ_On_Button(2*120, 2*124, 2*200, 2*158,2,2,10,2,0,0,0,0,JianCha);    //检查
			...
			{
				//rINTMSK|=BIT_EINT1;
				Lcd_Button(2*120,2*165, 2*200, 2*199,10, 2,INSERT);
				Lcd_DispHZ_On_Button(2*120,2*165, 2*200, 2*199,2,2,10,2,0,0,0,0,FanHui);    //返回
...
char ChangeScreenNum(char srcnum,char flag,INT16U b[])
{
	...
	if(flag==0)                                // +
	{
		...
		else
		{
			...
			Lcd_Fill_Box(2*120, 2*86, 2*200, 2*117,13);
			Lcd_DispAsc_On_Button(2*120, 2*86, 2*200, 2*117,strlen(str),13,2,1,0,0,str);
		}
	}
	else                                        // -
	{
